...
//		this.clusterCenters = clusterCenters;
	...
	public void doKmeans(int clusterCount){
		...
		for(int i=0;i<clusterCount;i++) 
			center[i] = new Cluster( new Pixel
									(random.nextInt(256),random.nextInt(256),random.nextInt(256)
							)); //losujê wartoœci centroidów w skali szaroœci
		...
		for(int i=0;i<clusterCount;i++){
			...
			for(int j=0;j<center[i].clusterArray.size();j++){
				this.img.setRGB(center[i].clusterArray.get(j).xPos, center[i].clusterArray.get(j).yPos, center[i].clusterColor.getRGB());
			}
		}
	}
